...
template<class Mat, class LockMat>
void conv(const Mat& A, const Mat& B, Mat& C, LockMat& M,
        ...
        for (std::size_t b_j = 0; b_j < B.width(); ++b_j) {
            ...
            for (std::size_t a_i = b_i; (a_i - b_i + B.height()) <= A.height(); a_i += stride) {
                for (std::size_t a_j = b_j; (a_j - b_j + B.width()) <= A.width(); a_j += stride) {
                    ...
                    /* Cの要素に対応したMの要素のlockメソッドを呼び出す */
                    M[c_i * C.width() + c_j].lock();
                    /* Cの要素に同時に読み書きできるスレッドは1つだけ */
                    C[c_i * C.width() + c_j]
                        += A[a_i * A.width() + a_j]
                        * B[b_i * B.width() + b_j];
                    /* Cの要素に対応したMの要素のunlockメソッドを呼び出す */
                    M[c_i * C.width() + c_j].unlock();
                }
            }
        }
    }
...
int main(int argc, char** argv)
{
    ...
    Matrix<std::mutex> M(output_width, output_height);
    std::mt19937 rd(0);
    ...
    C_multi.fill();
    const auto usec_multi = timer<std::chrono::microseconds>([&A, &B, &C_multi, &M, &stride]() { conv(A, B, C_multi, M, stride); } );
